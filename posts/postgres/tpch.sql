...
select * from  t40 where t40.a in (select a from t41);


* 分区键关联
      * 进行聚合操作， 则使用 Partial Aggregate
      * 不聚合，

* 不知道上层会使用子查询的结果做什么，所以需要 reduce 所有的数据
* 先使用分区键的关联条件过滤结果， 然后结果做reduce，但是当前advance reduce 会提前reduce，条件中的参数没有值，这种执行计划无法执行
